...
def extract_sb3_evaluations(log_dir: str):
    ...
    try:
        ...
        std_returns = (
            results.std(axis=1) if results.ndim > 1 else np.zeros_like(mean_returns)
        )
...
def get_model_info(log_dir: str) -> dict:
    ...
    if os.path.exists(best_model_dir):
        ...
        if best_files:
            info["best_model"] = os.path.join(
                "best_model", os.path.basename(best_files[0])
            )
...
def print_summary(results: dict, verbose: bool = False):
    ...
    if results["sb3_evaluations"] is not None:
        ...
        if len(df) > 0:
            print(
                f"  Melhor retorno: {df['mean_return'].max():.4f} (timestep: {df.loc[df['mean_return'].idxmax(), 'timestep']})"
            )
            print(f"  Último retorno: {df['mean_return'].iloc[-1]:.4f}")
            print(
                f"  Retorno médio: {df['mean_return'].mean():.4f} ± {df['mean_return'].std():.4f}"
            )
            if verbose:
...
def save_metrics(results: dict, output_dir: str, format: str = "json"):
    ...
    if format == "json":
        ...
        if results["sb3_evaluations"] is not None:
            output_data["sb3_evaluations"] = results["sb3_evaluations"].to_dict(
                "records"
            )

        if results["custom_evaluations"] is not None:
            output_data["custom_evaluations"] = results["custom_evaluations"].to_dict(
                "records"
            )
