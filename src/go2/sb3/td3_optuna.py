...
def find_latest_checkpoint(trial_log_dir: str) -> tuple[str | None, int]:
    ...
    for f in checkpoint_files:
        try:
            # Extract number from filename like td3_go2_50000_steps.zip
...
def objective(trial, args):
    ...
    # Suggest hyperparameters for TD3
    if not resume_from_checkpoint:
        ...
        target_policy_noise = trial.suggest_float("target_policy_noise", 0.1, 0.5, log=False)
        target_noise_clip = trial.suggest_float("target_noise_clip", 0.3, 0.7, log=False)
        
        # Action noise
        action_noise_sigma = trial.suggest_float("action_noise_sigma", 0.05, 0.3, log=False)
    ...
    try:
        ...
        mean_return = evaluate_td3_policy(eval_env, model, n_episodes=args.n_eval_episodes)
