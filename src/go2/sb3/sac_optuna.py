...
def evaluate_sac_policy(
    env, model, n_episodes: int = 10
) -> float:
    """
    Evaluate a SAC policy by running n_episodes and returning the mean return.
    """
    returns = []
...
def find_latest_checkpoint(trial_log_dir: str) -> tuple[str | None, int]:
    ...
    SB3 saves checkpoints as sac_go2_<timesteps>_steps.zip
    """
    if not os.path.exists(trial_log_dir):
        return None, 0
    
    # SB3 CheckpointCallback saves as <name_prefix>_<timesteps>_steps.zip
    checkpoint_files = glob.glob(os.path.join(trial_log_dir, "sac_go2_*_steps.zip"))
    
    if not checkpoint_files:
        # Also check for final model
        final_model = os.path.join(trial_log_dir, "sac_final.zip")
        if os.path.exists(final_model):
            return final_model, -1  # -1 indicates final model
        return None, 0
    
    # Extract timestep numbers and find the latest
    timesteps = []
    for f in checkpoint_files:
        try:
            # Extract number from filename like sac_go2_50000_steps.zip
            basename = os.path.basename(f)
            timestep_str = basename.replace("sac_go2_", "").replace("_steps.zip", "")
...
def objective(trial, args):
    """
    Optuna objective function that suggests hyperparameters, trains SAC, and returns evaluation metric.
    ...
    if resume_from_checkpoint and checkpoint_timestep == -1:
        # Final model exists, just evaluate
        print(f"Trial {trial.number}: Final model exists, evaluating...")
        try:
            model = SAC.load(checkpoint_path, device=args.device)
            mean_return = evaluate_sac_policy(eval_env, model, args.n_eval_episodes)
    ...
    if not resume_from_checkpoint:
        ...
        use_auto_ent_coef = trial.suggest_categorical("use_auto_ent_coef", [True, False])
        if use_auto_ent_coef:
            ent_coef = "auto"
        else:
        ...
        # Load hyperparameters from saved file
        try:
    ...
    if resume_from_checkpoint and checkpoint_timestep > 0:
        ...
        print(f"Trial {trial.number}: Loaded checkpoint, {remaining_timesteps} timesteps remaining")
    else:
        # Create new model
        model = SAC(
            "MlpPolicy",
            train_env,
            verbose=0,  # Set to 0 to reduce output during optimization
    ...
    try:
        ...
        # Save final model
        model.save(os.path.join(trial_log_dir, "sac_final"))
        
        # Evaluate the trained policy
        mean_return = evaluate_sac_policy(eval_env, model, n_episodes=args.n_eval_episodes)
        ...
        # Save current state
        model.save(os.path.join(trial_log_dir, "sac_interrupted"))
        train_env.close()
        eval_env.close()
...
def main():
    parser = argparse.ArgumentParser(
        description="Optuna hyperparameter optimization for SAC with Stable-Baselines3"
    )
    parser.add_argument("-e", "--exp_name", type=str, default="go2-sb3-sac-optuna")
    parser.add_argument(
        "--total_timesteps",
        type=int,
        default=200_000,
    ...
    if study.best_trial is not None:
        ...
        # Copy best model to main log directory
