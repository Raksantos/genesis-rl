...
def draw_joystick(
    image, lin_x, lin_y, max_lin_x, max_lin_y, radius=100, x_offset=10, y_offset=10
):
    for i in range(radius):
        r = radius - i
        color = (
            int(55 + 200 * (0.5 + 0.5 * i / radius)),
            int(55 + 200 * (0.5 + 0.5 * i / radius)),
            int(55 + 200 * (0.5 + 0.5 * i / radius)),
        )
    ...
    cv2.circle(
        image, (joystick_x + 2, joystick_y + 2), int(radius * 0.12), (0, 0, 0), -1
    )

    return image


def draw_target_height_bar(
    image, base_height, max_base_height, target_height=1.0, x_offset=220, y_offset=10
):
    ...
    current_height_pos = int(
        bar_y + bar_height - (base_height / max_base_height) * bar_height
    )
    cv2.rectangle(
        image,
        (bar_x, current_height_pos),
        (bar_x + bar_width, bar_y + bar_height),
        (0, 255, 0),
        -1,
    )

    target_height_pos = int(
        bar_y + bar_height - (target_height / target_height) * bar_height
    )
    cv2.line(
        image,
        (bar_x, target_height_pos),
        (bar_x + bar_width, target_height_pos),
        (0, 0, 255),
        2,
    )
...
def draw_angular_velocity_bar(image, ang_z, max_ang_z, x_offset=10, y_offset=220):
    ...
    cv2.rectangle(
        image, (bar_x, bar_y), (current_ang_pos, bar_y + bar_height), (0, 255, 0), -1
    )
...
def create_video_with_overlay(
    images_buffer, commands_buffer, output_video_path, fps=30
):
    ...
    max_lin_x, max_lin_y, max_ang_z, max_base_height, _ = normalize_commands(
        commands_buffer
    )
    ...
    for i in range(len(images_buffer)):
        ...
        image = draw_target_height_bar(
            image,
            base_height,
            max_base_height,
            x_offset=x_offset + radius * 2 + 10,
            y_offset=y_offset,
        )

        image = draw_angular_velocity_bar(
            image,
            ang_z,
            max_ang_z,
            x_offset=x_offset,
            y_offset=y_offset + radius * 2 + 20,
        )
